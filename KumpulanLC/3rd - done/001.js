...
STORE 'result' with 0 (str)

FOR each value of 'arrNum'
...
function addEvenOdd (arrNum) {
  ...
  for (var i = 0; i < arrNum.length; i++) {
    if (arrNum[i] % 2 === 1) {
      resultGanjil += arrNum[i];
    } else {
      resultGenap += arrNum[i];
    }
  }
  
  if (arrNum.length === 0) {
    arr[0].push(0); arr[1].push(0);
  } else {
    arr[0].push(resultGanjil); arr[1].push(resultGenap);
  }
  return arr;
}
