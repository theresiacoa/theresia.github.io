...
function profitCalculator(buyer) {
  ...
  for (var i = 0; i < buyer.length; i++) {
    ...
    for (var j = 0; j < listMenu.length; j++) {
      if (buyer[i].menu === listMenu[j][0]) {
        var gula = listMenu[j][1] * buyer[i].amount;
        var kopi = listMenu[j][2] * buyer[i].amount;
        
        if (gula <= stockGula && kopi <= stockKopi) {
          // console.log('======');
          var stockGula = stockGula - gula;
          var stockKopi = stockKopi - kopi;

          obj[buyer[i].menu].total += buyer[i].amount;
          obj[buyer[i].menu].profit += listMenu[j][3] * buyer[i].amount;
          obj[buyer[i].menu].time.push(buyer[i].waktu);
        }
      }
    }
  }
