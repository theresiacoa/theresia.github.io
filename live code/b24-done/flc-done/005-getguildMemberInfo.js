...
function getGuildMemberInfo(members) {
  ...
  for (var i = 0; i < members.length; i++) {
    obj.averageLevel += members[i].level;
    obj[members[i].class] = [];
  }
  obj.averageLevel = Math.floor(obj.averageLevel / members.length);
  // console.log(obj);

  //array
  for (var i = 0; i < members.length; i++) {
    if (members[i].level >= obj.averageLevel) {
      obj[members[i].class].push({ name: members[i].name, level: members[i].level });
    }
  }

  //search for blank category
  for (var i = 0; i < members.length; i++) {
    if (obj[members[i].class].length === 0) {
      delete obj[members[i].class];
    }
  }
