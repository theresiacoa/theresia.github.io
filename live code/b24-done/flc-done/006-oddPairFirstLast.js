...
function oddPairFirstLast(num) {
  ...
  if (numStr.length === 1) {
    var puluhan = numStr[0] + '1';
    if (puluhan % 2 === 1) {
      return Number(puluhan);
    } else {
      return 0;
    }
  } else {
    puluhan = numStr[0] + numStr[numStr.length-1];
  }

  if (puluhan % 2 === 0) {
    return oddPairFirstLast(numStr.substring(1, numStr.length-1));
  } else {
    return Number(puluhan) + oddPairFirstLast(numStr.substring(1, numStr.length-1));
  }
