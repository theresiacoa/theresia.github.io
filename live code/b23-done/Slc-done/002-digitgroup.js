...
//function menerima array of number 1 d
//returnnya array multidimension
//ada 4 array utk angka 1 digit 2 digit dst sampai 4
/*
ALGORITHM
1. Buat variable 'hasil' dengan nilai empat array kosong didalam array
2. 
*/
...
function digitsGrouper(numbers) {
  ...
  for (var i = 0; i < numbers.length; i++) {
    if (numbers[i] < 10) {
      arr[0].push(numbers[i]);
    } else if (numbers[i] < 100) {
      arr[1].push(numbers[i]);
    } else if (numbers[i] < 1000) {
      arr[2].push(numbers[i]);
    } else if (numbers[i] < 10000) {
      arr[3].push(numbers[i]);
    }
  }
  return arr;
}
