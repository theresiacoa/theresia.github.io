...
function oddPairFirstLast(num) {
  if (num === String(num)) {
    ...
    for (let i = 0; i < strNum.length/2; i++) {
      if (i != strNum.length-1-i) {
        kelompok.push(strNum[i] + strNum[strNum.length-1-i]);
      }
      else {
        kelompok.push(strNum[i] + '1')
      }
    }
    var result = 0;
    for (let i = 0; i < kelompok.length; i++) {
      if (Number(kelompok[i]) % 2 !== 0) {
        result += Number(kelompok[i])
      }
    }
    return oddPairFirstLast(String(result));
  }
}
