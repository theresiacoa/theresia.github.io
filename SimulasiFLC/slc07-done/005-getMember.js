...
function getMemberSummary(member) {
  ...
  for (var i = 0; i < member.length; i++) {
    ...
    if (obj.oldest_member === undefined && obj.youngest_member === undefined) {
      obj.oldest_member = {full_Name: member[i].first_name +' '+ member[i].last_name, age: member[i].age};
      obj.youngest_member = {full_Name: member[i].first_name +' '+ member[i].last_name, age: member[i].age};
    } else {
      if (obj.oldest_member.age < member[i].age) {
        obj.oldest_member = {full_Name: member[i].first_name +' '+ member[i].last_name, age: member[i].age};
      }
      if (obj.youngest_member.age > member[i].age) {
        obj.youngest_member = {full_Name: member[i].first_name +' '+ member[i].last_name, age: member[i].age};
      }
    }
    average+= member[i].age;
  }
  average = Math.round(average / member.length);
