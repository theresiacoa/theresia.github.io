/*
  ...
  yang terjadi saat asian games berlangsung. 
  
  Pelanggaran akan terhitung apabila
...
function ganjilGenapAsianGames(date, data) {
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    var checkPlat = plat[1][plat[1].length - 1];
    if (date < 32 && date > 0) {
      if (date % 2 === 0 && checkPlat % 2 !== 0 && data[i].type === 'Mobil') {
        counter++;
      } else if (date % 2 === 1 && checkPlat % 2 === 0 && data[i].type === 'Mobil') {
        counter++;
      }
    } else {
      return 'invalid date';
