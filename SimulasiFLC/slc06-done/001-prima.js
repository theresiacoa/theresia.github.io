...
STORE 'arr' with blank array

FOR each value of 'num' with increment index(i) by 1
  STORE 'isPrima' with TRUE
  FOR each value of 'num' with increment index(j) by 1
    IF index(i) value equals to index(j) value AND index(i) is not equals to index(j) THEN
      SET 'isPrima' with FALSE
    END IF
  END FOR

  IF 'isPrima' is TRUE then
    ADD index(i) value to 'arr'
  END IF
END FOR
